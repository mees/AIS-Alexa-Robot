...
class AudioIntegrationTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        def custom_token_intents():
            return audio("playing with custom token").play(self.stream_url, opaque_token=self.custom_token)
    ...
    def test_play_intent_with_custom_token(self):
        ...
        data = json.loads(response.data.decode("utf-8"))
        self.assertEqual("playing with custom token", data["response"]["outputSpeech"]["text"])
