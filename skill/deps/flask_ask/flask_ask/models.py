...
from .core import context, current_stream, dbgdump, session, stream_cache
...
class _Response(object):
        ...
        directive = [
            {
                ...
                "template": {"type": template, "backButton": backButton, "title": title, "listItems": listItems},
            }
        ]

        if background_image_url is not None:
            directive[0]["template"]["backgroundImage"] = {"sources": [{"url": background_image_url}]}
    ...
    def display_render(
        ...
        directive = [
            {
                "type": "Display.RenderTemplate",
                "template": {"type": template, "backButton": backButton, "title": title, "textContent": text},
            }
        ]
    ...
    def render_response(self):
        response_wrapper = {"version": "1.0", "response": self._response, "sessionAttributes": session.attributes}
            ...
            "directives": [
                {
                    "type": "Connections.SendRequest",
                    "name": "Upsell",
                    "payload": {"InSkillProduct": {"productId": productId}, "upsellMessage": msg},
                    "token": "correlationToken",
                }
            ],
        }
...
class audio(_Response):
    ...
    def enqueue(self, stream_url, offset=0, opaque_token=None):
        ...
        audio_item = self._audio_item(
            stream_url=stream_url, offset=offset, push_buffer=False, opaque_token=opaque_token
        )
