...
from flask_ask import Ask, audio, current_stream, logger, question, statement
...
def pause():
    ...
    msg = "Paused the Playlist on track {}, offset at {} seconds".format(queue.current_position, seconds)
    _infodump(msg)
    dump_stream_info()
...
def dump_stream_info():
    status = {"Current Stream Status": current_stream.__dict__, "Queue status": queue.status}
    _infodump(status)
